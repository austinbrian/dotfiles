...
# ZSH_THEME="clean"
# ZSH_THEME="powerlevel9k/powerlevel9k"  # deprecated, replaced by p10k
...
# Legacy p9k config (kept for reference, p10k uses ~/.p10k.zsh instead)
...
daylog() {
  ...
  case "${1:-today}" in
    tomorrow)  target_date=$(date -v+1d +%Y-%m-%d) ;;
    yesterday) target_date=$(date -v-1d +%Y-%m-%d) ;;
    today)     target_date="$today" ;;
    last)      target_date=$(ls "$daylog_dir"/*.md 2>/dev/null | sed 's|.*/||;s|\.md$||' | sort | awk -v t="$today" '$0 < t' | tail -1) ;;
    next)      target_date=$(ls "$daylog_dir"/*.md 2>/dev/null | sed 's|.*/||;s|\.md$||' | sort | awk -v t="$today" '$0 > t' | head -1) ;;
    *)         target_date="$1" ;;
  esac
